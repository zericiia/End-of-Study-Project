...
router.post("/:id", verifyTokenAndStudent, async (req, res) => {
  ...
  if (studentProposals >= 3) {
    return res.status(403).json({ message: "You can submit a maximum of 3 proposals" })
  }
...
router.get(
  "/viewProposal/:id",
  verifyTokenAndAuthorization({ roles: ["Admin", "Student"], userIdParam: "id" }),
  async (req, res) => {
    try {
 ...
 */
router.put("/:proposalId", verifyTokenAndStudent, async (req, res) => {
  try {
 ...
 */
router.delete("/:proposalId", verifyTokenAndStudent, async (req, res) => {
  try {
    const { proposalId } = req.params
...
/**
 * @desc  Update proposal status (accept/decline)
 * @route PUT /api/proposals/:proposalId/status
 * @method put
 * @access Private (only the teacher who owns the project)
 */
// added new
router.put("/:proposalId/status", verifyTokenAndTeacher({ roles: ["Teacher", "Admin"] }), async (req, res) => {
  try {
    const { proposalId } = req.params
    ...
    if (req.user.role === "Teacher" && proposal.project.teacher.toString() !== req.user.id) {
      return res.status(403).json({ message: "You are not allowed to update this proposal's status" })
    }
